...
function App() {
  ...
  const handleClear = () => {
    ...
    if (window.confirm('🗑️ Clear all conversation history?')) {
      setMessages([]);
      setText("");
      setError("");
  ...
  return (
    <div className="app">
        ...
        <div className="header-content">
          <div className="header-left">
            ...
            <div className="header-title">
              ...
              <p>Your intelligent AI-powered assistant</p>
            </div>
          </div>
          ...
          <div className="task-section">
            <div className="task-options-grid">
              {[
                ...
                <button
                  key={option.value}
                  className={`task-option-card ${task === option.value ? 'selected' : ''}`}
        ...
        <div className="messages-container">
          {messages.length === 0 ? (
            <div className="empty-state">
              ...
              <h2>Welcome to AI Smart Assistant</h2>
              <p>Your intelligent conversation partner</p>
              <div className="example-questions">
                <p>💡 Try asking:</p>
                <div className="example-chips">
                  <button onClick={() => {
                    setText("What is machine learning?");
                    textareaRef.current?.focus();
                  }}>
                    What is ML?
                  </button>
                  <button onClick={() => {
                    setText("How to make a perfect pizza?");
                    textareaRef.current?.focus();
                  }}>
                    How to make pizza?
                  </button>
                  <button onClick={() => {
                    setText("Write a Python function to sort a list");
                    textareaRef.current?.focus();
                  }}>
                    Write Python code
                  </button>
            ...
            messages.map((msg, index) => (
              <div 
                key={msg.id || index} 
                className={`message ${msg.type} ${getMessageStyle(msg)}`}
              >
                <div className="message-header">
                  <span className="message-avatar">
                    {msg.type === 'user' ? '👤' : getTaskEmoji()}
                  </span>
                  <span className="message-label">
                    {msg.type === 'user' ? 'You' : getTaskLabel()}
                  </span>
                  {msg.task && msg.type === 'ai' && (
                    <span className="message-task-tag">{msg.task}</span>
                  )}
                  <span className="message-time">{formatTime(msg.timestamp)}</span>
                </div>
                <div className="message-content">
                  {msg.type === 'ai' && msg.status === 'loading' ? (
      ...
      <footer>
        <p>AI Smart Assistant • Created by Yash • {messages.length} messages</p>
      </footer>
